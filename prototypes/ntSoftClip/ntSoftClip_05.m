classdef ntSoftClip_05 < audioPlugin
   ...
   methods (Static)
      function y = soft_clip_asym(x, g_up, g_up_recipr, g_dn, g_dn_recipr, alpha)
         % y = x;
         % y(x > 0) = x(x > 0) .* g_up .* 0.6667;
         % y(x < 0) = x(x < 0) .* g_dn .* 0.6667;
         % y(x > 1) = 1;
         % y(x < (- 1)) = -1;
         % a = y(y < 1);
         % b = y(y > (-1));
         % c = a(b == 1);
         % y(c == 1) = (1 + alpha) .* x(c == 1) - alpha .* x(c == 1) .^ 3;
         % y(x > 0) = y(x > 0) .* g_up_recipr;
         % y(x < 0) = y(x < 0) .* g_dn_recipr;
         y = x;
         if x(1) > 0
            x(1) = x(1) .* g_up;
         else
