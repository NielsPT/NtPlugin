classdef ntSoftClip_02 < audioPlugin
   properties
      bypass = false;
   ...
   methods (Static)
      function y = soft_clip(x, g, g_recipr, alpha)
         y = x;
         x = x .* g;
         if x(1) > 1
            y(1) = 1;
      ...
      function y = process(p, x)
         ...
         for i = 1:n
            ...
            if p.bypass_oversampling
               y(i, :) = p.soft_clip(x_local, g_cl, g_cl_recipr, alpha) .* g;
               continue;
            end
            ...
            if p.oversampling_uses_iir
               ...
               for j = 1:L
                  ...
                  acc_cl = p.soft_clip(accum .* L, g_cl, g_cl_recipr, alpha);
