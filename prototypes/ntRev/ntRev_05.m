classdef ntRev_05 < audioPlugin
  properties
    ...
    dLineEarly = zeros(ntRev_05.dLineLen * 2, 2);
    dLineLate = zeros(ntRev_05.dLineLen, 2, 8);
    dLineLateDel = zeros(ntRev_05.dLineLen, 2);
    nEarly = zeros(64, 2);
    nLate = zeros(8, 2);
    nLateDel = 1;
    gEarlyDiffuse = zeros(64, 1);
    stLocLate = 1;
    stLocEarly = 1;
    gLate = zeros(8, 2);
    Q = hadamard(8);
  ...
  methods
      ...
      for i = 1:n
        if p.bypass == true
          ...
          for j = 1:64
            ...
            yEarly(2) = yEarly(2) + p.dLineEarly(loc, 2) .* p.gEarlyDiffuse(j);
            loc = p.stLocEarly - p.nEarly(j, 2) - p.nPre;
            loc(loc < 1) = loc + ntRev_05.dLineLen;
            yEarly(1) = yEarly(1) + p.dLineEarly(loc, 1) .* p.gEarlyDiffuse(j);
          end
          if p.serial == true
            xLate = yEarly;
          else
    ...
    function update(p)
      ...
      diffuse = [gEarly; repmat(gEarly, 7, 1) * p.diffusion];
      p.gEarlyDiffuse = reshape(diffuse, 1, []);
    end
    function reset(p)
