...
NTFX_TEST() {
  ...
  auto peakDbScLink        = NtFx::Comp::PeakSideChainDb(peakSettings1);
  ...
  auto rmsScDb        = NtFx::Comp::RmsSideChainDb(rmsSettings);
  NTFX_ADD_TEST(rmsScDb, "dynamic_alternating");
  auto rmsScLin = NtFx::Comp::RmsSideChainLinear(rmsSettings);
