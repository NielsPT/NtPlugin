...
static const std::vector<std::string> STIMULI_NAMES {
  ...
  /**
   * @brief Set of a number of tests. Represents a full test file and this a
   * test of a Component in the library or an NtPlugin.
   *
  ...
  struct ComponentTest {
     ...
     * Accepted values are defined in STIMULI_NAMES. Defaults to an empty
     * vector, selecting all knows stimuli as defined in STIMULI_NAMES.
     */
    ...
    /**
     * @brief Processes the object under test with 'stimulus', writes out
     * results to a file and compares to expected results if found.
     *
    ...
    bool _compareExpected(
      ...
      for (size_t i = 0; i < y.size(); i++) {
        ...
        if (diff > acceptedDiff) {
          std::cout << this->owner.name << "." << this->objName << "."
                    << stimulus << ":" << " output: {" << y[i].l << ", "
                    << y[i].r << "}, expected: {" << e[i].l << ", " << e[i].r
