...
template <typename signal_t> struct Stereo {
  signal_t l{0};
  signal_t r{0};
  Stereo() : l(0.0f), r(0.0f) {}
  Stereo(signal_t x) : l(x), r(x) {}
  Stereo(signal_t left, signal_t right) : l(left), r(right) {}
  NTFX_INLINE_MEMBER Stereo<signal_t> &
  operator=(const Stereo<signal_t> &x) noexcept {
    this->l = x.l;
    this->r = x.r;
    return *this;
  }
...
NTFX_INLINE_TEMPLATE Stereo<signal_t> operator+(const Stereo<signal_t> &y,
                                                const signal_t &x) noexcept {
  return {(y.l + x), (y.r + x)};
}
NTFX_INLINE_TEMPLATE Stereo<signal_t> operator-(const Stereo<signal_t> &y,
                                                const signal_t &x) noexcept {
...
NTFX_INLINE_TEMPLATE Stereo<signal_t>
operator+(const Stereo<signal_t> &y, const Stereo<signal_t> &x) noexcept {
...
NTFX_INLINE_TEMPLATE Stereo<signal_t>
operator+(const signal_t &x, const Stereo<signal_t> &y) noexcept {
  return {(x + y.l), (x + y.r)};
}
NTFX_INLINE_TEMPLATE Stereo<signal_t>
operator-(const signal_t &x, const Stereo<signal_t> &y) noexcept {
...
NTFX_INLINE_TEMPLATE Stereo<signal_t>
ensureFinite(Stereo<signal_t> x, signal_t def = NTFX_SIG_T(0)) noexcept {
